...
/**
 * Initialize proxy state for a stream session and start connecting to the upstream.
 *
 * Allocates and configures the session's upstream context, applies local bind and
 * socket options, prepares downstream buffers and event handlers, evaluates a
 * configured upstream address (if present), resolves or selects the appropriate
 * upstream server, and initiates the upstream connection or name resolution.
 *
 * @param s Stream session to proxy. The function stores allocated upstream state
 *          in s->upstream and sets session-level handlers and timers as needed.
 */
static void
...
/**
 * Complete upstream connection setup for a session and start proxying data.
 *
 * Performs post-connection initialization for the session's upstream:
 * configures TCP_NODELAY, records connect time, notifies the peer,
 * allocates and attaches the upstream buffer, moves any downstream
 * preread data into the upstream output chain, prepends a PROXY
 * protocol v1 header when requested, computes upload/download rate
 * limits, marks the upstream as connected, installs upstream event
 * handlers, and begins the proxy processing loop.
 *
 * On resource allocation or socket configuration failures this routine
 * advances to the next upstream or finalizes the session by calling
 * ngx_stream_proxy_next_upstream() or ngx_stream_proxy_finalize().
 *
 * @param s ngx_stream_session_t*  The stream session whose upstream is being
 *                                initialized; modified in-place (upstream
 *                                state, buffers, chains, handlers, and timers).
 */
static void
...
/**
 * Handle a downstream (client) connection event and drive proxy data processing.
 *
 * Processes the proxy connection for the session associated with the provided
 * event. The event's `write` flag is used to indicate whether the handler
 * should process the write direction.
 *
 * @param ev Event for the downstream connection; its `write` field denotes write-direction processing.
 */
static void
...
/**
 * Advance the session to the next upstream attempt or finalize the session.
 *
 * Performs failover for the session's upstream: rejects the current peer if
 * necessary, enforces UDP-specific clearing of outgoing buffers, checks retry
 * limits and next-upstream timeout, closes and records the current upstream
 * connection state, and either initiates a new upstream connection or
 * finalizes the session with an appropriate error.
 *
 * @param s The stream session whose upstream should be advanced.
 */
static void
...
/**
 * Finalize a stream proxy session.
 *
 * Performs upstream cleanup (cancels pending name resolution, updates state
 * timing and byte counters, releases the upstream peer and closes the
 * upstream connection when present) and then finalizes the stream session
 * with the provided result code.
 *
 * @param s The stream session to finalize.
 * @param rc The finalization return code to pass to ngx_stream_finalize_session.
 */
static void
...
/**
 * Allocate and initialize a ngx_stream_proxy_srv_conf_t with default (unset) values.
 *
 * The returned structure is zeroed and its configurable fields are initialized to
 * the module's NGX_CONF_UNSET_* sentinel values so they may be merged later.
 *
 * @returns Pointer to the initialized ngx_stream_proxy_srv_conf_t, or `NULL` on allocation failure.
 */
static void *
...
/**
 * Merge server-level proxy configuration values, inheriting unspecified
 * settings from the parent and applying module defaults.
 *
 * Merges timeouts, buffer size, rate limits, request/response limits,
 * retry and connection behavior flags, local bind settings, and socket
 * options from `parent` into `child` when those fields are unset.
 *
 * @param cf Configuration context (unused by this function but required by API).
 * @param parent Pointer to the parent (previous) server configuration.
 * @param child Pointer to the child (current) server configuration to merge into.
 *
...
/**
 * Parse and configure the `proxy_pass` directive for a stream server.
 *
 * Compiles the provided upstream address; if it contains variables, stores a
 * compiled complex value in the server configuration, otherwise registers the
 * static upstream. Also assigns the stream handler that will use the configured
 * upstream.
 *
 * @param cf The configuration parsing context.
 * @param cmd The directive definition (unused by this function).
 * @param conf Pointer to the server-level proxy configuration (`ngx_stream_proxy_srv_conf_t *`).
 *
 * @returns `NGX_CONF_OK` on success, `NGX_CONF_ERROR` on failure, or the string
 *          `"is duplicate"` if `proxy_pass` is configured more than once.
 */
static char *
